	...
	public NuGetModule(ReadOnlyTargetRules Target) : base(Target)
	{
		...
		if (Target.Platform == UnrealTargetPlatform.Win64 || Target.Platform == UnrealTargetPlatform.HoloLens)
		{
			...
			// run nuget to update the packages
			{
				...
				int ExitCode = 0;
				var StdOut = Utils.RunLocalProcessAndReturnStdOut(NugetExe, string.Format("install \"{0}\" -OutputDirectory \"{1}\"", Path.Combine(ModuleDirectory, "packages.config"), NugetFolder), out ExitCode, true);
				if (ExitCode < 0)
				{
			...
			if (!string.IsNullOrEmpty(CppWinRTPackage))
			{
				...
				//var ExitCode = Utils.RunLocalProcessAndPrintfOutput(StartInfo);
				int ExitCode = 0;
				var StdOut = Utils.RunLocalProcessAndReturnStdOut(CppWinRTExe, string.Format("{0} -input \"{1}\" -output \"{2}\"", WinMDFilesStringbuilder, Target.WindowsPlatform.WindowsSdkVersion, CppWinRTFolder), out ExitCode, true);               
				if (ExitCode < 0)
				{
